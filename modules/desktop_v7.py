...
from core.financial_engine import analyse_financial_quality
...
anup_financials = anup_data["financial_quality"]["historical_financials"]

financial_analysis = analyse_financial_quality(anup_financials)
...
def show_company_dossier(company_data):
    

    data = company_data
    financial_analysis = analyse_financial_quality(
    data["financial_quality"]["historical_financials"]
    )
    ...
    score_frame = tk.Frame(dossier)
    score_frame.pack(fill="x", padx=15, pady=10)
    score = data["scorecard"]

    tk.Label(
        score_frame,
        text="EIOS CIO DASHBOARD",
        font=("Arial", 16, "bold")
    ).pack(anchor="w")

    tk.Label(
        score_frame,
        text=f"Overall Grade : {score['overall_grade']}"
    ).pack(anchor="w")

    tk.Label(
        score_frame,
        text=f"Investment Thesis : {score['investment_thesis']}"
    ).pack(anchor="w")

    tk.Label(
        score_frame,
        text=f"Recommended Action : {score['action']}"
    ).pack(anchor="w")
    tk.Label(
        score_frame,
        text="EIOS SCORECARD",
    ...
    tk.Label(
        score_frame,
        text=f"Business Quality : {data['business_quality']}",
        font=("Arial", 11)
    ).pack(anchor="w")

    tk.Label(
        score_frame,
        text=f"Management : {data['management']}",
        font=("Arial", 11)
    ).pack(anchor="w")

    tk.Label(
        score_frame,
        text=f"Moat : {data['moat']}",
        font=("Arial", 11)
    ).pack(anchor="w")

    tk.Label(
        score_frame,
        text=f"Financial Quality : {data['financial_quality']}",
        font=("Arial", 11)
    ).pack(anchor="w")

    tk.Label(
        score_frame,
        text=f"Valuation : {data['valuation']}",
        font=("Arial", 11)
    ).pack(anchor="w")
...
{financial_analysis["confidence"]}


STRENGTHS

{financial_analysis["financial_strengths"]}


WEAKNESSES

{financial_analysis["financial_weaknesses"]}
...
def company_registry():
    ...
    for company in companies:   
            ...
            width=40,
            height=2,
            command=lambda c=company: show_company_dossier(c)
        ).pack(pady=10)
