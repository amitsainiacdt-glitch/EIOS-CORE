...
class SignalInterpretationLedgerTests(unittest.TestCase):
 def setUp(self):
  ...
  self.conversion=HistoricalComparisonEvidenceConversionReceipt(1,"candidate-1","c"*64,"ev-1","Statement","Source","Historical Comparison","Supportive",90.0,90.0,2,True,False,"Notes","Reviewer",self.when,"Assessor",self.when,"Converter",self.when)
  self.interpretation=SignalInterpretation(title="Revenue inflection",description="A governed comparison supports an emerging revenue inflection.",detected_date="2026-08-21",domain=SignalDomain.EARNINGS,signal_type=SignalType.INFLECTION,direction=SignalDirection.POSITIVE,stage=SignalStage.EMERGING,horizon=TimeHorizon.MEDIUM_TERM,companies=["Entity A"],economic_mechanism="Demand supports revenue growth.",supply_demand_impact="Demand improves.",earnings_impact="Revenue may improve.",valuation_impact="No valuation conclusion.",magnitude=70.0,probability=75.0,persistence=60.0,relevance=90.0,market_expectation="Not assessed automatically.",market_recognition=20.0,price_reaction="Not assessed automatically.",causal_chain=["Demand","Revenue"],beneficiaries=["Entity A"],historical_precedent="Governed historical comparison.",invalidation_conditions=["Demand weakens"])
 def test_round_trip_without_signal_creation(self):
  with TemporaryDirectory() as temp:
   ...
   self.assertEqual(record.interpretation.relevance,90.0)
